...
def resolve_amqp_config(params: EndpointParams, operation_name: str, app_id: str | None = None) -> AmqpConfig:
    """
    ...
    match (is_reply or channel.address is None, 
           amqp_binding,
           channel.address,
           operation_name):
        ...
        case (True, _, _, _):
            return AmqpConfig(
                ...
                routing_key=f"reply-queue-{app_id}" if app_id else "reply-queue-default",
                binding_type=AmqpBindingType.REPLY,
                queue_properties={"durable": True, "exclusive": False}
            )
            
        # AMQP queue binding pattern
        case (False, binding, _, _) if binding and hasattr(binding, 'type') and binding.type == "queue":
            return resolve_queue_binding(binding, param_values, channel, operation_name)
            
        # AMQP routing key binding pattern  
        case (False, binding, _, _) if binding and hasattr(binding, 'type') and binding.type == "routingKey":
            return resolve_routing_key_binding(binding, param_values, channel, operation_name)
            
        # AMQP exchange binding pattern
        case (False, binding, _, _) if binding and hasattr(binding, 'type') and binding.type == "exchange":
            return resolve_exchange_binding(binding, param_values, channel, operation_name)
...
def resolve_queue_binding(binding: Any, param_values: dict[str, str], channel: Channel, operation_name: str) -> AmqpConfig:
    ...
    match (getattr(binding, 'queue', None), channel.address, operation_name):
        case (queue_config, _, _) if queue_config and getattr(queue_config, 'name', None):
...
def resolve_routing_key_binding(binding: Any, param_values: dict[str, str], channel: Channel, operation_name: str) -> AmqpConfig:
    ...
    match (exchange_config and getattr(exchange_config, 'name', None), 
           channel.address, 
           operation_name):
...
def resolve_exchange_binding(binding: Any, param_values: dict[str, str], channel: Channel, operation_name: str) -> AmqpConfig:
