...
class CodeGenerator:
    ...
    def generate(self, spec_path: Path, output_dir: Path, force: bool = False) -> None:
        ...
        producer_nested_classes = self._collect_nested_classes(
            producer_routers, router_type="Producer"
        )
        consumer_nested_classes = self._collect_nested_classes(
    ...
    def _generate_nested_routers_code(
        ...
        for key, value in routers_dict.items():
            if isinstance(value, RouterInfo):
                ...
                subclass_name = (
                    f"{router_type}{key.title()}Router"
                    if router_type
                    else f"{key.title()}Router"
                )
                lines.append(
    ...
    def _collect_nested_classes(
        ...
        for key, value in routers_dict.items():
            if not isinstance(value, RouterInfo):
                ...
                classes.extend(
                    self._collect_nested_classes(value, full_prefix, router_type)
                )

        return classes

    def _generate_nested_class(
        self, class_name: str, routers_dict: Dict[str, Any], router_type: str = ""
    ) -> str:
