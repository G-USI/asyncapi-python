    ...
    try:
        with abs_path.open('r', encoding='utf-8') as f:
            ...
            if not isinstance(data, dict):
                raise ValueError(f"Expected YAML document to be a dictionary, got {type(data)}")
...
def resolve_reference(ref_data: YamlDocument) -> YamlDocument:
    ...
    if not isinstance(resolved_data, dict):
        raise ValueError(f"Reference {ref_string} resolved to non-dictionary: {type(resolved_data)}")
    ...
    def wrapper(data: YamlDocument) -> T:
        if is_reference(data):
            ...
            if target_context.filepath != current_context.filepath:
                ...
                push_context(target_context.with_pointer(""))  # Start at root of new file
                try:
