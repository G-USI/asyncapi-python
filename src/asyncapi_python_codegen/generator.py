...
class CodeGenerator:
    ...
    def generate(self, spec_path: Path, output_dir: Path, force: bool = False) -> None:
        ...
        self._generate_file("messages.py.j2", messages_json_dir / "__init__.py", context)
    ...
    def _build_routers(self, operations: Dict[str, Operation]) -> List[RouterInfo]:
        ...
        for op_id, operation in operations.items():
            ...
            path = tuple(segment for segment in clean_op_id.replace("/", ".").split(".") if segment)
            
            # Generate router class name - clean up any invalid characters
            class_name = "".join(segment.title().replace("-", "").replace("_", "") for segment in path) + "Router"
        ...
        if hasattr(message, "payload") and isinstance(message.payload, dict):
            ...
            if payload.get("type") == "object" and "properties" in payload:
                for prop_name, prop_schema in payload["properties"].items():
                    field_info = {
                        "type": self._json_type_to_python(prop_schema.get("type", "Any")),
                        "default": None,
                    }
                    ...
                    # Handle enum
                        ...
                        enum_vals = ", ".join(json.dumps(v) for v in prop_schema["enum"])
    ...
    def _generate_file(self, template_name: str, output_path: Path, context: Dict[str, Any]) -> None:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    print(f"  Error: All Black formatting attempts failed for {filename}: {e3}")
