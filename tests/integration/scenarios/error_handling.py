...
async def error_handling(wire: AbstractWireFactory, codec: CodecFactory) -> None:
    ...
    try:
        ...
        try:
            ...
            print(f"⚠ UserApp - Only {len(messages_consumed)}/{expected_messages} messages consumed")
    ...
    try:
        ...
        try:
            ...
            print(f"✓ OrderApp - All {len(order_messages_consumed)} order events consumed")
        except asyncio.TimeoutError:
            print(f"⚠ OrderApp - Only {len(order_messages_consumed)}/{expected_order_messages} order events consumed")
        
        # Log RPC replies consumed
        if replies_consumed:
            print(f"✓ OrderApp - Consumed {len(replies_consumed)} RPC replies from default queue")
    ...
    for model_class, message_name in [
        ...
        if model_class == TestUser:
            ...
            test_data = model_class(user_id=999, name="Test", email="test@example.com", timestamp="2024-01-01T00:00:00Z")
            encoded = test_codec.encode(test_data)
            decoded = test_codec.decode(encoded)
            ...
            test_data = model_class(event_type="test", user_id=999, timestamp="2024-01-01T00:00:00Z")
            encoded = test_codec.encode(test_data)
            decoded = test_codec.decode(encoded)
