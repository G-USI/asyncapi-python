...
class EventBroadcaster(BaseApplication):
    ...
    def _setup_endpoints(self):
        ...
        service_names = ["EmailService", "SmsService", "PushNotificationService", "AnalyticsService", "AuditService"]
...
class BaseConsumerService(BaseApplication):
    ...
    def __init__(self, service_name: str, wire_factory: AbstractWireFactory, codec_factory: CodecFactory):
...
async def fan_out_broadcasting(wire: AbstractWireFactory, codec: CodecFactory) -> None:
    ...
    print(f"Testing fan-out broadcasting with {wire.__class__.__name__} + {codec.__class__.__name__}")
    ...
    consumer_services = [email_service, sms_service, push_service, analytics_service, audit_service]
    ...
    async def handle_email_user_action(action: UserAction):
        ...
        total_received += 1
        print(f"✓ EmailService received: {action.action_type} for user {action.user_id}")
    ...
    async def handle_push_user_action(action: UserAction):
        ...
        total_received += 1
        print(f"✓ PushNotificationService received: {action.action_type} for user {action.user_id}")
        if total_received >= expected_total_events:
            consume_event.set()
    ...
    try:
        ...
        try:
            ...
            print(f"⚠ Only {total_received}/{expected_total_events} events consumed within timeout")
        
        # Verify each service received all events
        for service_name, events in received_events.items():
            assert len(events) == expected_events_per_service, f"{service_name} should have received {expected_events_per_service} events, got {len(events)}"
            ...
            assert event_types == expected_types, f"{service_name} received events in wrong order: {event_types}"
            
            # Verify user IDs match
            user_ids = [event.user_id for event in events]
            expected_user_ids = [123, 456, 789]
            assert user_ids == expected_user_ids, f"{service_name} received wrong user IDs: {user_ids}"
        
        print(f"✓ All {len(consumer_services)} consumer services received events correctly")
        ...
        async def simulate_processing(service_name: str, processing_time: float):
            ...
            processing_results[service_name] = f"Processed {len(received_events[service_name])} events"
        ...
        assert len(processing_results) == len(consumer_services), "Not all services completed processing"
        for service_name in received_events.keys():
            assert service_name in processing_results, f"{service_name} did not complete processing"
