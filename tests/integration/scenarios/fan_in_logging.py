...
async def fan_in_logging(wire: AbstractWireFactory, codec: CodecFactory) -> None:
    ...
    print(f"Testing fan-in logging with {wire.__class__.__name__} + {codec.__class__.__name__}")
    ...
    async def handle_log_event(log: LogEvent):
        ...
        print(f"✓ LogAggregator received: {log.service_name} [{log.level}] {log.message}")
    ...
    try:
        ...
        try:
            ...
            print(f"⚠ Only {len(received_logs)}/{expected_log_count} log messages consumed within timeout")
        
        # Verify we received logs from all services
        services_logged = set(log.service_name for log in received_logs)
        expected_services = {"UserService", "OrderService", "PaymentService", "NotificationService"}
        assert services_logged == expected_services, f"Missing logs from services: {expected_services - services_logged}"
        ...
        assert len(trace_logs) >= 4, f"Should have at least 4 logs with trace_id {trace_id}, got {len(trace_logs)}"
        
        # Verify log distribution across services
        log_counts_by_service = {}
        for log in received_logs:
            log_counts_by_service[log.service_name] = log_counts_by_service.get(log.service_name, 0) + 1
