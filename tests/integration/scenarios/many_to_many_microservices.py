...
class OrderServiceApp(BaseApplication):
    ...
    def _setup_endpoints(self):
        ...
        self.order_placed_endpoints["payment"] = self._register_endpoint(payment_operation)
        self.order_placed_endpoints["inventory"] = self._register_endpoint(inventory_operation)
...
class PaymentServiceApp(BaseApplication):
    ...
    def _setup_endpoints(self):
        ...
        self.publish_payment_processed = self._register_endpoint(payment_processed_operation)
...
class InventoryServiceApp(BaseApplication):
    ...
    def _setup_endpoints(self):
        ...
        self.publish_inventory_updated = self._register_endpoint(inventory_updated_operation)
...
async def many_to_many_microservices(wire: AbstractWireFactory, codec: CodecFactory) -> None:
    """Test many-to-many microservices scenario with complex service interactions"""
    print(f"Testing many-to-many microservices with {wire.__class__.__name__} + {codec.__class__.__name__}")
    ...
    all_services = [user_service, order_service, payment_service, inventory_service, shipping_service]
    ...
    order_statuses = {}  # order_id -> {"payment": bool, "inventory": bool, "shipped": bool}
    ...
    async def handle_user_created(user: UserCreated):
        ...
        if user.user_id not in [99999, 99998, 99997]:
            print(f"ⓘ OrderService: Ignoring user from other test: {user.name} (ID: {user.user_id})")
            return
        ...
        order = OrderPlaced(
            ...
            items=[{"sku": "item-123", "quantity": 2}, {"sku": "item-456", "quantity": 1}],
            total_amount=199.99,
            timestamp="2024-01-01T00:00:00Z"
        )
        
        # Initialize order status tracking
        order_statuses[order.order_id] = {"payment": False, "inventory": False, "shipped": False}
    ...
    async def handle_order_for_payment(order: OrderPlaced):
        ...
        print(f"✓ PaymentService: Processing payment for order {order.order_id} (${order.total_amount})")
    ...
    async def handle_order_for_inventory(order: OrderPlaced):
        # Only process our test orders
        if order.user_id not in [99999, 99998, 99997]:
            print(f"ⓘ InventoryService: Ignoring order from other test: {order.order_id}")
            return
        ...
        inventory = InventoryUpdated(
            order_id=order.order_id,
            items_reserved=[{"sku": item["sku"], "quantity": item["quantity"], "reserved": True} for item in order.items],
    ...
    async def handle_payment_processed(payment: PaymentProcessed):
        # Only process our test orders
        if not (payment.order_id.startswith("order-99999") or payment.order_id.startswith("order-99998") or payment.order_id.startswith("order-99997")):
            print(f"ⓘ ShippingService: Ignoring payment from other test: {payment.order_id}")
            return
    ...
    async def handle_inventory_updated(inventory: InventoryUpdated):
        # Only process our test orders
        if not (inventory.order_id.startswith("order-99999") or inventory.order_id.startswith("order-99998") or inventory.order_id.startswith("order-99997")):
            print(f"ⓘ ShippingService: Ignoring inventory from other test: {inventory.order_id}")
            return
    ...
    async def _check_and_ship_order(order_id: str):
        ...
        if order_id in order_statuses:
            ...
            if status["payment"] and status["inventory"] and not status["shipped"]:
                ...
                print(f"✓ ShippingService: Order {order_id} shipped with tracking {shipped_order.tracking_number}")
    ...
    try:
        ...
        print(f"✓ Order {order_id} completed full workflow: User → Order → Payment & Inventory → Shipping")
        ...
        users = [
            UserCreated(user_id=99998, name="Bob Smith MultiTest", email="bob@example.com", timestamp="2024-01-01T01:00:00Z"),
            UserCreated(user_id=99997, name="Carol Brown MultiTest", email="carol@example.com", timestamp="2024-01-01T01:00:01Z")
        ]
        
        # Publish users concurrently
        await asyncio.gather(*[user_service.publish_user_created(user) for user in users])
        ...
        assert len(events_received["final_shipped_orders"]) >= 2, f"Expected at least 2 shipped orders, got {len(events_received['final_shipped_orders'])}"
        
        print(f"✓ Successfully processed {len(events_received['final_shipped_orders'])} concurrent orders")
        print("✓ Many-to-many microservices scenario completed successfully")
        
    except asyncio.TimeoutError as e:
        print(f"⚠ Workflow timeout - some services may not have processed events in time")
